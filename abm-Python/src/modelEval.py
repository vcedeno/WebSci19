...
sys.path.insert(0, os.getcwd()+'/src/h2')
...
def main():
    ...
    for i in range(numlines):
    	...
    	if function=='h2':
    		...
    		phasefile=os.getcwd()+functions[i]["phase"]
    		actionfile=os.getcwd()+functions[i]["action"]
    		...
    		for j in range(nodes):
    			...
    			players_info[agent_id].append(player_id)
    			players_info[agent_id].append(agent_id)
    			
    		#print(players_info)
    		h2.main(graphfile,beta,nodes,edges,timeSeconds,seed_value,iteration,players_info,word_dist,letter_req_dist,frac_rpl_dist,time_rpl_dist,experiment_id,actionfile,phasefile)
